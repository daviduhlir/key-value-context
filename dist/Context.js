...
class Context {
    constructor(topData = {}, config = {}) {
        ...
        this.storageKey = this.config.sharedClusterKey ? this.config.sharedClusterKey : utils_1.getRandomId();
        this.initialize();
    }
    ...
    async runInContext(handler) {
        ...
        let error;
        try {
            result = await Context.stackStorage.run(Object.assign(Object.assign({}, storage), { [this.storageKey]: [...stack, actualData] }), async () => handler());
        }
        catch (e) {
            error = e;
        }
...
Context.stackStorage = new AsyncLocalStorage_1.default();
